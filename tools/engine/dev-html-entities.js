...
"icon": "&",
"kind": "code",
"description": "Escape text for safe display in HTML, or convert entities back to characters.",
"keywords": ["html entities","html escape","html encode","htmlspecialchars","escape html"],
"inputLabel": "Text or HTML",
"outputLabel": "Result",
"placeholder": "<script>alert(\"hi\")</script>",
"sample": "<a href=\"/tools\">Tools & Calculators</a> — 100% free",
"options": [{"key":"dir","label":"Direction","type":"select","default":"enc","options":[{"value":"enc","label":"Escape →"},{"value":"dec","label":"← Unescape"}]}],
"transform": (text, { dir }) => {
      if (!text) return { output: '', note: 'Type or paste something above.' };
      const map = { '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' };
      if (dir === 'enc') {
        const output = text.replace(/[&<>"']/g, c => map[c]);
        return { output, stats: [['Characters escaped', String((text.match(/[&<>"']/g) || []).length)]] };
      }
      const rev = { amp: '&', lt: '<', gt: '>', quot: '"', '#39': "'", apos: "'", nbsp: '\u00a0' };
      const output = text.replace(/&(#x?[0-9a-fA-F]+|[a-zA-Z]+);/g, (m, e) => {
        if (rev[e] !== undefined) return rev[e];
        if (e[0] === '#') {
      ...
      return { output, stats: [['Entities decoded', String((text.match(/&\w+;|&#x?\w+;/g) || []).length)]] };
    },
"tips": ["Escaping user input before putting it in a page is the single most effective defence against cross-site scripting.","Escape & first when doing it by hand, or you will double-escape the other entities.","Inside an HTML attribute you must escape quotes as well as angle brackets."],
"faq": [{"q":"Is escaping enough to stop XSS?","a":"For text placed between tags, largely yes. Content injected into a script block, a style block, an event handler or a URL attribute needs context-specific escaping — HTML escaping alone will not protect those positions."}]
};
})();