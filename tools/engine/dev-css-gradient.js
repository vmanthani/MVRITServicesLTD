...
"inputLabel": null,
"outputLabel": "CSS",
"fields": [{"key":"type","label":"Type","type":"select","default":"linear","options":[{"value":"linear","label":"Linear"},{"value":"radial","label":"Radial"},{"value":"conic","label":"Conic"}]},{"key":"angle","label":"Angle (degrees)","type":"number","default":120,"min":0,"max":360},{"key":"c1","label":"Colour 1","type":"color","default":"#ffe29a"},{"key":"c2","label":"Colour 2","type":"color","default":"#f7c948"},{"key":"c3","label":"Colour 3 (optional)","type":"color","default":"#ff9d2e"}],
"generate": (f) => {
      const stops = [f.c1, f.c2, f.c3].filter(Boolean);
      const list = stops.map((c, i) => `${c} ${Math.round((i / Math.max(1, stops.length - 1)) * 100)}%`).join(', ');
      const value = f.type === 'linear' ? `linear-gradient(${f.angle}deg, ${list})`
                  : f.type === 'radial' ? `radial-gradient(circle at 50% 50%, ${list})`
                  : `conic-gradient(from ${f.angle}deg at 50% 50%, ${list})`;
      return {
        output: `background: ${value};`,
        preview: value,
        stats: [['Colour stops', String(stops.length)], ['Type', f.type]]
      };
    },
"tips": ["Gradients between complementary hues pass through grey in the middle. Keep the stops close on the colour wheel for a cleaner blend.","Add a plain background-color above the gradient as a fallback for very old browsers.","Large gradients can band on 8-bit displays. A subtle noise overlay or an extra midpoint stop hides it."],
"faq": [{"q":"Why does 0deg point upward?","a":"In CSS gradients the angle names the direction the gradient travels towards, measured clockwise from up. So 0deg runs bottom to top, and 90deg runs left to right."}]
};
})();