...
"icon": "</>",
"kind": "code",
"description": "Indent, tidy and check XML. Also minifies for production and reports tag structure.",
"keywords": ["xml formatter","xml beautifier","xml validator","pretty print xml","tidy xml"],
"inputLabel": "XML",
...
"options": [{"key":"mode","label":"Output","type":"select","default":"pretty","options":[{"value":"pretty","label":"Formatted (indented)"},{"value":"minify","label":"Minified"}]},{"key":"indent","label":"Indent","type":"select","default":"2","options":[{"value":"2","label":"2 spaces"},{"value":"4","label":"4 spaces"},{"value":"tab","label":"Tab"}]}],
"transform": (text, { mode, indent }) => {
      if (!text.trim()) return { output: '', note: 'Paste some XML above.' };
      const check = checkXmlBalance(text);
      if (check.error) return { error: check.error };
      const pad = indent === 'tab' ? '\t' : ' '.repeat(Number(indent));
      const output = mode === 'minify' ? minifyXml(text) : formatXml(text, pad);
      return {
        output,
        stats: [
          ['Tags balanced', 'yes'],
          ['Elements', String(check.elements)],
          ['Max depth', String(check.depth)],
          ['Input', bytes(text)],
          ['Output', bytes(output)]
        ]
      };
    },
"tips": ["This checks that every tag is opened and closed in the right order. It does not validate against a DTD or XSD schema.","XML is case-sensitive: <Item> and <item> are different elements.","Five characters must be escaped in text content: & < > \" and '."],
"faq": [{"q":"What is the difference between well-formed and valid XML?","a":"Well-formed means the syntax is correct — tags nest and close properly. Valid means it also conforms to a schema that defines which elements are allowed where. This tool checks well-formedness."}]
};
})();