...
window.DEV_TOOLS["case-converter"] = {
...
"transform": (text, { target }) => {
      ...
      const conv = line => {
        ...
        if (!w.length) return '';
        switch (target) {
          case 'camel':    return w.map((x, i) => i ? cap(x) : x.toLowerCase()).join('');
          ...
          case 'title':    return w.map((x, i) => (i && SMALL.has(x.toLowerCase())) ? x.toLowerCase() : cap(x)).join(' ');
