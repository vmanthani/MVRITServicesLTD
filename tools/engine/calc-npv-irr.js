...
"inputs": [{"key":"initial","label":"Initial investment","type":"number","unit":"£","default":100000,"min":0},{"key":"flows","label":"Cash flows per period (comma separated)","type":"text","default":"30000, 35000, 40000, 45000, 50000"},{"key":"rate","label":"Discount rate (cost of capital)","type":"number","unit":"%","default":10,"step":0.1}],
"compute": ({ initial, flows, rate }) => {
      const cf = String(flows).split(/[\s,;]+/).map(Number).filter(n => isFinite(n));
      ...
      let run = -initial;
      table.rows.push(['0', fmtC(-initial), '1.0000', fmtC(-initial), fmtC(run)]);
      cf.forEach((c, i) => {
        const f = 1 / Math.pow(1 + r, i + 1);
        run += c * f;
        table.rows.push([String(i + 1), fmtC(c), f.toFixed(4), fmtC(c * f), fmtC(run)]);
      });
