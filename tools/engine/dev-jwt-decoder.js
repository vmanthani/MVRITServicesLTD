...
"icon": "🎫",
"kind": "code",
"description": "Decode a JSON Web Token to inspect its header, payload and expiry. Runs entirely in your browser.",
"keywords": ["jwt decoder","decode jwt","json web token","jwt viewer","jwt payload"],
"inputLabel": "JWT",
...
"transform": (text) => {
      ...
      const stats = [
        ...
        ['Expires', when(payload.exp)]
      ];
      if (payload.exp) stats.push(['Status', payload.exp < now ? 'EXPIRED' : 'Valid for ' + humanDuration(payload.exp - now)]);

      return {
        output: '// Header\n' + JSON.stringify(header, null, 2) + '\n\n// Payload\n' + JSON.stringify(payload, null, 2),
        stats,
        warn: 'This decodes only — it does not verify the signature. Never trust an unverified token to make an access decision.'
      };
    },
"tips": ["A JWT is signed, not encrypted. Anyone holding the token can read the payload, so never put secrets in it.","iat, exp and nbf are seconds since 1970 UTC, not milliseconds — a common off-by-1000 bug.","Signature verification needs the secret or public key and must happen on your server, never in a browser."],
"faq": [{"q":"Is it safe to paste a real token here?","a":"Decoding happens entirely in your browser and nothing is transmitted or stored. That said, a live token is a live credential — prefer an expired or test token, and rotate anything you paste into any online tool."}]
};
})();